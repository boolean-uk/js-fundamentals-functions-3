...
console.log(timerStatus(123))
console.log(timerStatus('abc'))
...
function estimatePrepTime(ingredients, prepTime) {
  let estimatedTime
  if (prepTime > 0) {
    estimatedTime = prepTime * ingredients.length
  } else {
    estimatedTime = 2 * ingredients.length
  }
  return estimatedTime
}
console.log(estimatePrepTime(['sugar', 'milk', 'flour', 'eggs'], 3))
...
function improveRecipe(ingredients, portion) {
  ...
  for (const iterator of Object.entries(ingredients)) {
    ingredients[iterator[0]] = iterator[1] * portion
  }
  return ingredients
}
