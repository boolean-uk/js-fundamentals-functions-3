...
function estimatePrepTime(ingredients, prepTime = 2) {
  ...
  for (let i = 0; i < ingredients.length; i++) {
    wholePrepTime += prepTime
  }
  return wholePrepTime
}
...
function calculateQuantities(ingredients, numLayers) {
  ...
  if (ingredients.includes('sugar')) {
    requiredIngredients.sugar = 100 * numLayers
  }
  if (ingredients.includes('eggs')) {
    requiredIngredients.eggs = 2 * numLayers
  }
...
function improveRecipe(ingredients, portion) {
  ...
  for (const [key, value] of Object.entries(ingredients)) {
    updatedIngredients[key] = value * portion
  }
